...
async function LinearSearch() {
	let delay = Disable_The_Input();
	let targetInput = document.getElementById("targetValue").value;
	if (targetInput == "") {
	...
	updateStatus(`Searching for ${target} (Linear Search)...`);

	let found = false;
	for (let i = 0; i < bars.length; i++) {
		let curr_id = bars[i].split('id="')[1].split('"')[0];
		let val = parseInt(bars[i].split(/[:%]/)[1]);

		document.getElementById(curr_id).style.backgroundColor = selected;
		let sound = MapRange(val, 2, 100, 500, 1000);
		beep(100, sound, delay);

		await Sleep(delay);

		if (val === target) {
			document.getElementById(curr_id).style.backgroundColor = finished;
			found = true;
...
async function BinarySearch() {
	...
	let target = parseInt(targetInput);
	updateStatus(`Searching for ${target} (Binary Search)...`);
	...
	while (left <= right) {
		...
		let mid_id = bars[mid].split('id="')[1].split('"')[0];
		let val = parseInt(bars[mid].split(/[:%]/)[1]);

		// Visualize range
		for (let i = left; i <= right; i++) {
			let id = bars[i].split('id="')[1].split('"')[0];
			document.getElementById(id).style.backgroundColor = chng;
		}

		document.getElementById(mid_id).style.backgroundColor = selected;
		let sound = MapRange(val, 2, 100, 500, 1000);
		beep(100, sound, delay);
		await Sleep(delay * 2);

		if (val === target) {
			document.getElementById(mid_id).style.backgroundColor = finished;
			found = true;
		...
		for (let i = 0; i < bars.length; i++) {
			if (i != mid) { // Don't reset if found (handled by found=true break)
				let id = bars[i].split('id="')[1].split('"')[0];
				document.getElementById(id).style.backgroundColor = def;
			}
		}
	...
	if (!found) {
		updateStatus(`${target} Not Found (check if sorted)!`);
		alert("Value not found (or array not sorted)!");
	}
	Finished_Sorting();
}
